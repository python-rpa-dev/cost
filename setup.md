...
Read actual token counts from the opencode SQLite database. Auto-detects the DB path across platforms. Applies pricing via whole-segment matching against `--pricing-file` entries, falling back to the file's `_default` key and then `_DEFAULT_PRICING`. When `obfuscate=True`, the DB path in output is truncated to the last 25 characters. Records are tagged `opencode@<env>` (`env` defaults to auto-detect: `wsl`/`windows`/`local`), so each OS's own database keeps an independent watermark; only records newer than that watermark are tracked. `since_ts`/`until_ts` bound a UTC epoch-seconds window (start inclusive, end exclusive). Returns newly tracked totals keyed by model.
