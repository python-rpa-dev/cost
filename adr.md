...
Storage: `data/token_log.json` (gitignored, dir auto-created) is the cumulative ledger — per-model usage plus a `_meta.watermark` map of `source@env` -> newest tracked epoch seconds; readers must skip keys starting with `_`. `pricing.json` supplies per-model overrides merged over `_DEFAULT_PRICING = (0.03, 0.05)` USD per million tokens.
