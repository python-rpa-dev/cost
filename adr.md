...
Shared core (the client-plugin seam): scanners normalize their source into `UsageRecord(key, input_tokens, output_tokens, ts, source)` tuples and tag them `source@env` (`_detect_env`: WSL env vars -> `wsl`, win32 -> `windows`, else `local`; CLI `--env` overrides), so per-OS databases sharing one repo folder never share a watermark; `_scan_report(records, overrides, monthly, since_ts, until_ts)` filters to the `[since_ts, until_ts)` window, gates records against per-key watermarks under the ledger's `_meta`, then — inside one FileLock read-modify-write — sums fresh records via `_aggregate` (per `(source@env, model)`, `TotalsEntry` totals + monthly buckets + time span), merges them into the log per model (sources merged), advances each watermark to its newest fresh timestamp, and atomically writes. Rendering: Source-column table (`Source / Model / Input / Output / Cost`, grand-total row `Total scanned`) plus `_print_monthly`/`_print_date_range`. `get_totals` keeps the plain 4-column model table; manual `track_tokens` shares the log via `_track_batch`.
...
- CLI: `totals | scan-oc | scan-pi | scan-all | clear | pricing | export`, flags `--pricing-file --obfuscate --no-monthly --db-path --sessions-dir --since --until --env`; `scan-all` composes both collectors into one Source-tagged report and returns newly tracked totals keyed `(source@env, model)`.

Storage: `data/token_log.json` (gitignored, dir auto-created) is the cumulative ledger — per-model usage plus `_meta`: a `watermark` map of `source@env` -> newest tracked epoch seconds and `months`, unpriced `month -> model -> [input, output]` buckets accumulated by every scan. Readers must skip keys starting with `_`; costs are always computed at display time. `pricing.json` supplies per-model overrides merged over `_DEFAULT_PRICING = (0.03, 0.05)` USD per million tokens.
...
- Idempotent scans per source-and-environment: timestamp watermark under `_meta`; records at/below it are skipped and reported (`Skipped N already-tracked record(s) from opencode@wsl.`), so re-scanning full-history sources never double-counts and WSL/Windows scans aggregate into one ledger. Undated records count once, on the key's first scan. `clear` resets ledger and watermarks together; renaming watermark keys (e.g. adding env suffixes) requires clear + rescan — stale keys are ignored, never inherited.
- Ledger-backed monthly report: scans persist raw month buckets (`_meta.months`); `get_totals` reprices them with current pricing and prints the cumulative Monthly Breakdown — reports stay queryable without re-scanning, and price changes reprice history (no stored costs).
...
- Watermark is monotonic per `source@env` key: a `--since` window on a fresh ledger permanently skips older history (the ledger only moves forward); accepted for the simple invariant — full rebuild is one `clear` + rescan away.
- Cross-environment sharing assumes sequential scans: `FileLock` over the WSL<->Windows 9P/UNC boundary is not a trustworthy mutex; concurrent scans from both OSes could interleave writes. Accepted — single-operator tool, natural usage is one scan at a time.
